...
const newsData = (data) => {
  ...
  data.map((news) => {
    ...
    content.innerHTML = `<span id="content">${news.newsContent.substring(0, 199)}</span>`;
    img.src = news.imageUrl;
    country.innerHTML = `Country : <span class="ctry">${news.Country}</span>`;
    like.innerHTML = `<img src="../Image/like2.png" alt="" class="img-fluid w-50">
    <span class="badge badge-light">${
      Variables[like] === undefined ? `00` : Variables[like]
    }</span>`;

    share.innerHTML = `<img src="../Image/share2.png" alt="" class="img-fluid w-50">
    <span class="badge badge-light">${
    ...
    DEL.className = "btn btn-secondary position-relative m-2";
    like.setAttribute("iconName", "Like");
    share.setAttribute("iconName", "Share"); 
    DEL.setAttribute("iconName", "DEL"); 
    
    div.append(title, img, content, country, like, share,DEL);
    DEL.addEventListener('click',(ele,i)=>{
      data.splice(i,1)
      newsData(data)
    })
